...
function sweep(){const t=now();for(const [token,s] of sessions){if(s.expiresAt<=t)sessions.delete(token)}for(const [key,r] of rate){if(t-r.start>=RATE_WINDOW_MS*2)rate.delete(key)}while(sessions.size>MAX_SESSIONS)sessions.delete(sessions.keys().next().value);while(rate.size>MAX_RATE_KEYS)rate.delete(rate.keys().next().value)}
...
async function body(req){let size=0,chunks=[];for await(const c of req){size+=c.length;if(size>MAX_BODY)throw new Error('BODY_TOO_LARGE');chunks.push(c)}let parsed;try{parsed=JSON.parse(Buffer.concat(chunks).toString('utf8')||'{}')}catch{throw new Error('BAD_JSON')}if(!parsed||typeof parsed!=='object'||Array.isArray(parsed))throw new Error('BAD_JSON');return parsed}
function client(state,id){if(!state.clients[id])throw new Error('BAD_CLIENT');return state.clients[id]}
function issueSession(id){sweep();const token=crypto.randomBytes(32).toString('base64url');sessions.set(token,{id,expiresAt:now()+SESSION_TTL_MS});if(sessions.size>MAX_SESSIONS)sessions.delete(sessions.keys().next().value);return {token,expiresIn:SESSION_TTL_MS/1000}}
...
export async function createDemoServer({stateFile=process.env.FIA_DEMO_STATE_FILE||''}={}){
  ...
  const server=http.createServer(async(req,res)=>{try{
    ...
    if(req.method==='POST'&&req.url==='/operations'){
      const p=await body(req),key=normalizeIdempotencyKey(req),result=await store.transact(s=>createOperation(s,actor,p,key));
