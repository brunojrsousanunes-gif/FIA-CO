...
const json=(res,code,data)=>{securityHeaders(res);res.setHeader('content-type','application/json; charset=utf-8');res.statusCode=code;res.end(JSON.stringify(data))};
...
export function createDemoServer(){return http.createServer(async(req,res)=>{try{
  if(!throttle(req))return json(res,429,{error:'RATE_LIMITED'});
  if(!checkOrigin(req))return json(res,403,{error:'ORIGIN_FORBIDDEN'});
  if(req.method==='GET'&&req.url==='/health')return json(res,200,{ok:true,mode:'demo',realFunds:false});
  if(req.method==='POST'&&req.url==='/session'){
    const p=await body(req),id=safe(p.clientId),secret=String(p.secret||''),cfg=secrets();client(id);
    ...
    state.audit.push({operationId:null,actor:id,event:'SESSION_CREATED',at:new Date().toISOString()});
  ...
  if(req.method==='GET'&&req.url==='/state')return json(res,200,snapshot(actor));
  if(req.method==='POST'&&req.url==='/operations')return json(res,201,createOperation(actor,await body(req)));
  const m=req.url?.match(/^\/operations\/([A-Z0-9-]+)\/decision$/);if(req.method==='POST'&&m)return json(res,200,decide(actor,m[1],(await body(req)).decision));
  return json(res,404,{error:'NOT_FOUND'});
}catch(e){const map={BODY_TOO_LARGE:413,BAD_JSON:400,BAD_CLIENT:400,SAME_CLIENT:400,BAD_AMOUNT:400,NO_BALANCE:409,BAD_CONCEPT:400,NOT_FOUND:404,FORBIDDEN:403,ALREADY_DECIDED:409,BAD_DECISION:400};return json(res,map[e.message]||400,{error:e.message||'BAD_REQUEST'})}})}
export function resetDemoState(){state.clients.A.balance=1250;state.clients.B.balance=850;state.operations.length=0;state.audit.length=0;sessions.clear();rate.clear()}
if(import.meta.url===`file://${process.argv[1]}`){if(!process.env.FIA_DEMO_SECRET_A||!process.env.FIA_DEMO_SECRET_B){console.error('Faltan FIA_DEMO_SECRET_A / FIA_DEMO_SECRET_B');process.exit(1)}const port=Number(process.env.PORT||8787);createDemoServer().listen(port,'127.0.0.1',()=>console.log(`FIA&CO hardened demo API en http://127.0.0.1:${port}`))}
