...
assert.equal(p.defaults.realBiometrics,false);
assert.equal(p.defaults.identityMethod,'non-biometric');
assert.equal(p.defaults.productionProvider,false);
assert.equal(p.useCases.length,8);
['universalEvidenceAdmissibility','realFundsExecutionOrCustody','realPiiCapture','bindingDisputeResolution','premiumEnforcement'].forEach(k=>assert.equal(p.classification[k],'BLOCKED',k));
assert.equal(p.classification.realBiometricCapture,'PERMANENTLY_EXCLUDED');
